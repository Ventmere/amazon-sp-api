...
pub async fn get_authorization_code(configuration: &configuration::Configuration, selling_partner_id: &str, developer_id: &str, mws_auth_token: &str) -> Result<crate::models::GetAuthorizationCodeResponse, Error<GetAuthorizationCodeError>> {
    ...
    local_var_req_builder = local_var_req_builder.query(&[("sellingPartnerId", &selling_partner_id.to_string())]);
    local_var_req_builder = local_var_req_builder.query(&[("developerId", &developer_id.to_string())]);
    local_var_req_builder = local_var_req_builder.query(&[("mwsAuthToken", &mws_auth_token.to_string())]);
