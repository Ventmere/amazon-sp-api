...
pub async fn delete_listings_item(configuration: &configuration::Configuration, seller_id: &str, sku: &str, marketplace_ids: Vec<String>, issue_locale: Option<&str>) -> Result<crate::models::ListingsItemSubmissionResponse, Error<DeleteListingsItemError>> {
    ...
    let local_var_uri_str = format!("{}/listings/2021-08-01/items/{sellerId}/{sku}", local_var_configuration.base_path, sellerId=crate::apis::urlencode(seller_id), sku=crate::apis::urlencode(sku));
...
pub async fn get_listings_item(configuration: &configuration::Configuration, seller_id: &str, sku: &str, marketplace_ids: Vec<String>, issue_locale: Option<&str>, included_data: Option<Vec<String>>) -> Result<crate::models::Item, Error<GetListingsItemError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetListingsItemError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
