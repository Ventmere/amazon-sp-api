...
pub async fn get_definitions_product_type(configuration: &configuration::Configuration, product_type: &str, marketplace_ids: Vec<String>, seller_id: Option<&str>, product_type_version: Option<&str>, requirements: Option<&str>, requirements_enforced: Option<&str>, locale: Option<&str>) -> Result<crate::models::ProductTypeDefinition, Error<GetDefinitionsProductTypeError>> {
    ...
    if let Some(ref local_var_str) = product_type_version {
        local_var_req_builder = local_var_req_builder.query(&[("productTypeVersion", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = requirements {
        local_var_req_builder = local_var_req_builder.query(&[("requirements", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = requirements_enforced {
        local_var_req_builder = local_var_req_builder.query(&[("requirementsEnforced", &local_var_str.to_string())]);
    }
